...
<% if (hasDatabase) { %> 
    datasource:
        ...
        url: <% if (prodDatabaseType == 'mysql') { %> jdbc:mysql://{DNS}/{DATABASE}?useUnicode=true&characterEncoding=utf8&useSSL=false <% } else { %> jdbc:jtds:sqlserver://{DNS}/{DATABASE}  <% } %> 
        username: {USER}
        password: {PWD}
        hikari:
