...
import fr.demo2.repository.FournisseurRepository;
...
public class FournisseurResource {
    ...
    @Timed
    public List<Fournisseur> getAllFournisseurs(@RequestParam(required = false, defaultValue = "false") boolean eagerload) {
        log.debug("REST request to get all Fournisseurs");
        return fournisseurRepository.findAllWithEagerRelationships();
    }
    ...
    public ResponseEntity<Fournisseur> getFournisseur(@PathVariable Long id) {
        ...
        Optional<Fournisseur> fournisseur = fournisseurRepository.findOneWithEagerRelationships(id);
